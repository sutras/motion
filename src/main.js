let unified, animation, animationend;

const EVENT_SCROLL = 'scroll';
const EVENT_RESIZE = 'resize';
const ATTR_DATA_MOTION = 'data-motion';
...
const WeakMap = WeakMap || (function() {
    function WeakMap() {
        this.keys = [];
        this.values = [];
    }
...
// animation动画属性
const aniAttrs = ['duration', 'timing-function', 'delay', 'iteration-count', 'direction', 'fill-mode'];
const oneAttrs = [PROP_CLASS, 'name'].concat( aniAttrs );
...
function motion( options ) {
    ...
    function getValueFromDataOrOptions( el, prop, isNumber ) {
        ...
        let val = el.hasAttribute( attr ) ? el.getAttribute( attr ) : options[ camelCase( prop ) ];
        return isNumber ? Number( val ) : val;
    }

    function createQueues( el ) {
        let queues, name, queue,
    ...
    function isVisible( el ) {
        let viewTop, viewBottom, top, bottom, expandTop, expandBottom;
